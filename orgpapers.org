...
+ ~:float~ :: This option followed by ~t~ or ~nil~ specifies whether
  this object (table, figure or any other special block) is a floating
  object which can be optimally placed by LaTeX or do you want it
  exactly where you have placed it in the document. In most cases, you
  would want these objects to float so that LaTeX can optimally place
  them.
