...
use it, create a directory with your username under ~/.emacs.d/, and
...
(often mapped to alt) keys.[fn:2] Control key is usually referred to as ~C-~
...
included into the document.[fn:3]
...
output.[fn:4] Every time you evaluate the code using ~C-c C-c~, the
...
want to restrict the Tables and Images within subsections.[fn:6]
...
[fn:2] Depending on the keyboard and the default configuration of the
...
[fn:3] For libraries and functions that you need to call, it is even
...
[fn:4] Of various image formats, I find that png files are most
...
[fn:5] Author of the odt exporter has chosen to develop the exporter
...
[fn:6] See http://lexfridman.com/blogs/research/2011/03/06/prevent-figures-from-floating-outside-sections-in-latex/
