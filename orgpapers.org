...
#+begin_src emacs-lisp
  ...
  bibretrieve auctex cdlatex citeproc coffee-mode consult
  counsel-projectile counsel-tramp counsel dired-subtree
  dired-hacks-utils dirtree ebib edit-server elfeed-goodies
  ace-jump-mode elfeed-score elfeed emacsql embark erc-hl-nicks
  erc-image eshell-git-prompt ess-R-data-view ctable
  ess-r-insert-obj ess-smart-equals ess-smart-underscore ess-view
  ess-view-data csv-mode ess flx-ido flx flycheck git-gutter+
  git-gutter highlight-indentation htmlize ido-completing-read+
  ido-vertical-mode iedit ivy-bibtex bibtex-completion biblio
  biblio-core ivy-prescient key-chord keycast kurecolor
  magit git-commit magit-section memoize move-text multi-web-mode
  multiple-cursors nameless noflet notmuch-labeler notmuch org-gcal
  alert log4e gntp org-sidebar org-ql f dash-functional
  org-sticky-header org-super-agenda ht ov parsebib pdf-tools peg
  persist persistent-scratch pinentry popup popwin powerline
  prescient pretty-hydra hydra lv projectile pkg-info epl
  quelpa-use-package quelpa queue rainbow-delimiters rainbow-mode
  remember-last-theme request-deferred request deferred
  simple-httpd smartparens string-inflection super-save swiper ivy
  tblui tablist magit-popup transient tree-mode ts s dash
  use-package bind-key visual-fill-column which-key windata
  with-editor yasnippet-snippets yasnippet zenburn-theme))
...
John Kitchin's org-ref (https://github.com/jkitchin/org-ref) package provides the best toolkit for adding citation support to org mode.
...
you may often need to clean/correct the downloaded entries. And, when
the bibliographic information in BibTex/Biber format is not available
...
Emacs also has excellent tools that can be used to build your database. Once you are comfortable using emacs, you will find that the Emacs tools are actually the most efficient and cleanest. Ebib gives a nice interface to manually enter the records. Bibretrieve and org-ref give you commands that can download BibTeX records directly from online databases.
...
If you have loaded [[https://github.com/vikasrawal/orgpaper/blob/master/research-toolkit.org][research-toolkit.org]], you will automatically have access to three custom classes: vreport, vmemoir and varticle. Of these, vmemoir and varticle are based on the [[https://ctan.org/pkg/memoir?lang=en][Memoir]] class, an extremely versatile LaTeX package that allows for many customisations.

While vmemoir and varticle classes should work for books and articles, feel free to look at [[https://github.com/vikasrawal/orgpaper/blob/master/research-toolkit.org][research-toolkit.org]] and customise them for your need.
...
There are many LaTeX packages that can be used to format tables and images. Having played around with many, I have narrowed down for my use to tabularx, which I combine with siunitx and booktabs. I use longtable for tables that run across multiple pages, and tabu when I need to use coloured text or backgrounds in tables.

Tables will be formatted using lines starting with [[http://orgmode.org/manual/LaTeX-specific-attributes.html][~#+attr_latex~]]. These are used to pass various arguments to
underlying LaTeX commands. We shall also use org [[http://orgmode.org/manual/Special-blocks.html][special blocks]] for advanced table layouts.
...
*** Using tabular environment                                    :noexport:

The default LaTeX ~tabular~ environment allows only a few column types. In particular, there is limited support in tabular environment for wrapping text in different types of columns. However, there are several latex packages that allow additional features. We will use booktabs and dcolumn along with tabular. These can be called using the following lines in the preamble.

~#+LATEX_HEADER: \usepackage{booktabs,dcolumn}~

The LaTeX [[https://ctan.org/pkg/memoir?lang=en][memoir]] class, which is a very versatile class for writing articles and books, includes functions provided by LaTeX packages like  booktabs, dcolumn and tabularx. This means that, if you are using memoir class, you can use functions provided by these packages without having to call them.
...
#+BEGIN_SRC org :results replace drawer
  ...
  ,#+ATTR_LATEX: :center t :booktabs t :float t
     | State          | {{{MCD(Average yield)}}} | {{{MCD(Average output)}}} | {{{MCD(Average income)}}} |
     |----------------+--------------------------+---------------------------+---------------------------|
...
|                |              {{{MCD(Average yield)}}} | {{{MCD(Average output)}}} | {{{MCD(Average income)}}} |
|----------------+---------------------------------------+---------------------------+---------------------------|
| Madhya Pradesh |                                   669 |                  13121.25 |                   7234.25 |
| Haryana        |                                   300 |                   2532.30 |                     234.3 |
| Punjab         |                                   260 |                  35232.45 |                   15235.0 |
