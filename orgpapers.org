...
of literature. Building a personal bibliographic database that is
integrated with your writing system will be particularly beneficial if
academic writing is going to be a medium to long-term occupation for
you.
...
+ With easy style specifications that you provide, the document will
  ...
  - Org-mode has a powerful LaTeX converter. The user is not required
    to write latex mark-up. Basis Org-mode mark-up is extremely
    simple, and can be mastered in very little time.
  ...
  - The setup also provides tools for producing odt/docx files. With
    well defined template files, you can get a fully formatted
    openoffice or MS-Word output.
...
+ To insert footnotes at any point, use ~C-c C-x f~
