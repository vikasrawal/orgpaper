...
used to overcome this limitation while exporting to LaTeX or
HTML. These functions are already included in my emacs configuration
in (~_configs/use-org-contrib.el~).
...
Three additional hacks are required for using these environments in
orgmode.
...
#+BEGIN_SRC org :results replace drawer
    ...
    , #+ATTR_LATEX: :align width=0.8\textwidth,colspec={lS[table-format=3.0,round-mode=off,add-integer-zero = false]S[table-format=5.2,round-mode=places,round-integer-to-decimal,add-integer-zero=true,table-alignment-mode=marker]}
    ,#+ATTR_LaTeX: :options [remark{Notes} = {This note goes below the table},label=none]
    ,#+ATTR_LATEX: :center :booktabs t :font \small
       | State          | {{{mc(Average yield)}}} | {{{mc(Average income)}}} |
       |----------------+-------------------------+--------------------------|
       | Madhya Pradesh |                     669 |                 13121.25 |
       | Haryana        |                     300 |                  2532.30 |
       | Punjab         |                     260 |                 35232.45 |
